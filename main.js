    ...
    foods: {
        "^minecraft": {
            "^item !model !bucket": {
                ...
                "Porkchop !cook": null,
                "cook Porkchop": null,
                "Rabbit !hide !foot !stew !cook": null,
                "Cook Rabbit !hide !foot !stew": null,
                pufferfish: null,
                "Tropical Fish": null,
                "Cod !cook": null,
                "Cooked Cod": null,
...
function sort_and_draw_image(image_array_in, width, add_color_data) {
    ...
    for (let i in image_array) {
        ...
        if (redie_lol_ye && !obj_tmp.disabled) {
            if (!obj_tmp.file_obj) continue;
            if (
                !obj_tmp.file_obj.short_name.startsWith("leather") ||
                obj_tmp.file_obj.short_name.endsWith("_overlay") ||
                obj_tmp.file_obj.short_name == "leather"
            ) {
                continue;
            }
